    ...
    <Wrapper>
      <div class="flex flex-row justify-center">
              ...
              <li v-for="guest in guestList" :key="guest.id">
                <div class="relative">
                  <router-link :to="{ name: 'GuestProfile', params: { id: guest.id } }">
                    <div
                        class="p-4 border-2 border-hrdc-blue font-bold text-hrdc-blue text-md rounded cursor-pointer transition-transform transform hover:scale-105 hover:duration-300 text-center">
                      {{ guest.firstName + " " + guest.lastName }}
                      <div class="flex flex-row justify-center">
                        <div :style="{ backgroundImage: getBackgroundColorHMIS(guest)}"
                             class="h-8 w-8 rounded-full"></div>
                        <div :style="{ backgroundImage: getBackgroundColorNoTrespass(guest)}"
                             class="h-8 w-8 rounded-full"></div>
                        <div :style="{ backgroundImage: getBackgroundColorServicesOnly(guest)}"
                             class="h-8 w-8 rounded-full"></div>
                      </div>
                    </div>
                  </router-link>
                  <button @click="removeGuest(guest)"
                          class="absolute top-0 right-0 bg-hrdc-blue hover:bg-red-500 text-white font-bold px-2 py-1 rounded-full transform transition duration-300 hover:scale-110">
                    X
                  </button>
                </div>
              </li>
        ...
        <div class="w-1/3 p-4 flex flex-col justify-start">
          <div class="mb-4">
            <button @click="checkoutAll" class="bg-hrdc-green text-white py-2 px-4 rounded-md hover:bg-hrdc-teal">
              Checkout All Guests
            </button>
          </div>
          <div class="mb-4">
            <CounterCard :title="guestList.length + ' Guests Checked In'"
                         :content="guestList.length + ' Overnight Stays'" class="w-full"/>
          </div>
